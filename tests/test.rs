...
#[unsafe_fn]
fn take_struct(SomeStruct { i, s } : SomeStruct, foo@SomeStruct { .. } : &mut SomeStruct) -> SomeStruct {
    let y : u32 = unsafe { std::mem::zeroed() };
    foo.i += 1;
    SomeStruct {
        i: i + foo.i + y,
...
fn main() {
    ...
    let mut s1 = SomeStruct{ i: 8, s: "DEF".into() };
    let s2 = unsafe { take_struct(SomeStruct{ i: 5, s: "ABC".into() }, &mut s1 ) };
    assert_eq!(s1.i, 9);
    assert_eq!(s2.i, 5+9);
